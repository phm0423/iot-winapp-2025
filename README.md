...
- `RichTextBox`: 포맷팅이 가능한 텍스트박스
    ...
    - ScrollBars: 컨트롤 오른쪽, 하단에 스크롤바가 생성
        - Both보다는 ForcedBoth, ForcedVerical 사용
...
- `SaveFileDialog`: 저장위치에 파일명 지정하는 다이어로그 창 컨트롤
    - DlgSave 이름으로 사용
    - 코딩으로 설정내용 작성
...
### C# 문법
- 기본문법
    1. 형변환 - [소스](./day04/Day04Sutdy/SyntaxWinApp01/FrmMain.cs)
    ...
    4. 파일입출력 - [소스](./day04/Day04Sutdy/SyntaxWinApp03/FrmMain.cs)
        - 파일 쓰기, 읽기 기능
...
- 기본 문법
    1. 델리게이트(대리자), 이벤트 - [소스](./day05/Day05Study/SyntaxWinApp01/FrmMain.cs)
        ...
        - event: delegate에게 '무슨일 발생하면 실행해!'라고 해주는 역활
        ...
        - Form 오픈 이벤트
            ...
            2. `FrmMain_Load` 이벤트: 폼이 로드(메모리 업)될때 처리할 내용
                - 생성자 이외에 초기화 할 것
                - 폼 오픈시 이벤트 중에서 가장 중요!!!
            3. FrmMain_Showm 이벤트: 폼이 바탕화면에 그려질때 이벤트
                - 잘 안씀
            4. FrmMain_Activated 이벤트: 폼이 활성화(바탕화면 가장 위에서 동작하는 상태)될때 처리될 이벤트
        ...
        - 실제 실무에서는
            - Form() 생성자, FrmMain_Load(), FrmMain_FormClosing()으로 대부분 작업
...
- 고급 문법
    ...
    5. 제너릭: Generic
        ...
        ```
        - 위 3 개의 메서드를 아래의 하나의 메서드로 퉁칠 수 있음
        - 대문자 T는 아무거나 사용해도 무방. 대문자한글자를 선호

        - 제네릭 클래스
        - where T: class -> 참조형(클래스)만 허용
        - where T: struct -> 값형식(기본 타입)
    ...
    6. 비동기, 스레드 - [소스](./day05/Day05Study/SyntaxWinApp03/FrmMain.cs)
        ...
        - `세번째` 해결방법
            ...
            - 응닶없음은 나타나지 않음. 진행바 처리
            - 로직복잡해도 소스코드는 간결
        - `네번째` 해결방법
            - BackgroundWorker 클래스 사용 - 내일처리
            - 최하 난이도
...
### 윈앱 컨트롤 5
- `BackgroundWorker`: 화면 뒷단에서 작업할 일중 스레드 처리가 되어야할 부분을 손쉽게 동작시켜주는 컨트롤(화면표시 X)
    - Bgw~ 이름을 사용
...
- `CheckBox`: 여러개에서 다수를 선택할 수 있는 컨트롤(라디어버튼과 동작이 반대)
    - Chk~로 이름 사용
    - Checked: 선택 여부
    - CheckState: 
        - Unchecked: 체크를 안한 상태
    ...
    - CheckedChanged: 기본에빈트, 체크상태가 변경되면 발생
...
- `ToolTip`: 각 컨트롤에 툴팁 아이콘 표시 컨트롤
    - 툴팁컨트롤 하나로 모든 컨트롤 툴팁 관리 기능
    - Form_Load에서 작성.

### C# 문법
- C# 기본
    - enum: 열거형, 필요한 목록을 수로된 키값을 문자로 변경해서 사용하는 구조체
    ...
    - 모달창, 모달리스창: 창위에 다른 창을 띄울때 접근권한 제어 형태
        ...
        - Modaless: 부모창과 상관업이 동작. 메인창을 언제나 닫을 수 있고, 메인창을 닫으면 모두 종료. 사용시 주의
...
- 윈앱 응용개발
    ...
    <img src="./image/cs0016.png" width="800">
...
- WPF 특징
    ...
    <img src="./image/cs0019.png" width="400">
...
- 각 프레임워크별 장점
    - 윈앱(윈폼) - 오래된 기술로 안정적. 상용/무료 서드파티컨트롤이 많이 존재. 혼자 UI 개발할 때는 시간절약
    - WPF
...
- WPF 컴포넌트(실무에서 쓰이는 UI컴포넌트, 윈앱) - Third Party
...
### WPF 개발방법 및 컨트롤 1
- **WPF 애플리케이션** 선택 - [소스](./day07/Day07Study/WpfSutdyApp01/MainWindow.xaml.cs)
    - 나머지는 동일
...
- WPF 실제개발법(XAML) - [소스](./day07/Day07Study/WpfStudyApp02/MainWindow.xaml)
    ...
    - WrapPanel: 컨트롤이 넓이 영역을 벗어나면 아래로 위치
        - 왼쪽부터 오른쪽으로 컨트롤 위치
...
- 데이터 바인딩 - [디자인](./day07/Day07Study/WpfStudyApp04/MainWindow.xaml), [소스](./day07/Day07Study/WpfStudyApp04/MainWindow.xaml.cs)
    ...
    - 바인딩 방식
        1. 직접코드 할당방식(전통적 방식)
            - CboCollection.ItemSource = furitItems
            - 기존 윈앱 방식과 동일
...
- 데이터베이스 데이터 바인딩
    ...
    3. 비하인드 코드에서 MySQL 소스 추가
        ...
        6. 트랜잭션객체: 옵션. 트랜잭션(INSERT, UPDATE, DELETE) 처리시 필요

    <img src="./image/cs0018.png" width="600">
...
- 컨트롤 디자인, 리소스 - [소스](./day08/Day08Study/WpfStudyApp04/MainWindow.xaml)
    ...
    - 리소스: 컨트롤의 공유화
        ...
        - Applicatoin.Resources에 선언한 리소스 > StaticResource
...
- 공식사이트 - https://mahapps.com/
    ...
    - 2011년 개발시작, 2014년 1.0 배포. 현재 2.4.10
    - Metro UI, Mordern UI - MS에서 시작한 디자인 스타일
    - 깔끔하고 입체감을 최소화 시킴

- 사용법 - [소스](./day08/Day08Study/WfpStudyApp05/MainWindow.xaml)
    1. NuGet 패키지 관리 >
        - MahApps.Metro 설치
...
### MahApps.Metro 프레임워크
- 컨트롤 사용법 - [소스](./day09/Day09Sutdy/WpfStudyApp05/MainWindow.xaml.cs)
    - ProgressBar, MetroProgressBar, ProgressRing
    - TabControl

### VS Tip
- 프로젝트는 제거해도 폴더와 파일은 그대로 남아있음

### WPF UI 프레임워크
- 개요
    - Fluent UI란 이름의 Modern UI의 한 스타일 UI 프레임워크
    - 2021년 1.0배포, 현재 버전 4.0.2

- 기본 사용법 - [소스](./day09/Day09Sutdy/WpfSutdyApp06/MainWindow.xaml)
    ...
    - App.xaml.cs 오픈 - [소스](./day09/Day09Sutdy/WpfStudyApp07/App.xaml.cs)
        ```cs
